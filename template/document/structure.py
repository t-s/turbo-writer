...
class RequestHandler(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if document_entity:
            # Handle add request
            item_type = None
            if self.request.get("add_text"):
                item_type = dao.TEXT
            elif self.request.get("add_single_variable"):
                item_type = dao.SINGLE_VARIABLE
            elif self.request.get("add_repeating_variable"):
            ...
            if item_type:
                try:
                    ...
                    error_msg = "Adding document item failed: {}".format(e)
            ...
            for item in items:
                ...
                if item.item_type == dao.SINGLE_VARIABLE:
                    item.variable_name = self.request.get("single_name.{}".format(index))
                elif item.item_type == dao.REPEATING_VARIABLE:
                    item.variable_name = self.request.get("repeating_name.{}".format(index))
                elif item.item_type == dao.TEXT:
                    item.text = self.request.get("text.{}".format(index))
                item.style_name = self.request.get("style_name.{}".format(index))
                item.flow_control = self.request.get("flow_control.{}".format(index))
                try:
                    item.put()
                except Exception as e:
            ...
            for request in self.request.arguments():
                ...
                change_position = re.match(r"change_position\.(\d)", request)
                if change_position:
                    old_index = int(change_position.group(1))
                    new_position_string = None
                    try:
                        new_position_string = self.request.get("position.{}".format(old_index))
                        new_position = int(new_position_string)
                    except:
                        error_msg = "Invalid position specified: {}".format(new_position_string)
                        break
                    if new_position < 1:
                        new_position = 1
                ...
                remove_position = re.match(r"remove_position\.(\d)", request)
                if remove_position:
    ...
    def render(self, template, document_entity, error_msg):
        ...
        jinja_template = ui.get_template(self, "template/document/structure.html")
        ...
        jinja_template_values["items"] = dao.get_document_items(document_entity)
        ...
        jinja_template_values["style_names"] = dao.get_style_names(template)
